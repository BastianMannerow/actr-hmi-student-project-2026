...
class SimulationSettingsDialog(QDialog):
    """Edit one headless scenario, including virtual or ROS environment mode."""
    ...
    def __init__(self, config: SimulationConfig, parent=None) -> None:
        ...
        self.ros_group = QGroupBox("ROS 2 TurtleBot Connection", content)
        ros_form = QFormLayout(self.ros_group)
        ros = self._source.ros
        self.ros_interface = QComboBox()
        self.ros_interface.addItem("iRobot Create actions (repository)", "irobot_actions")
        self.ros_interface.addItem("Generic cmd_vel", "cmd_vel")
        self.ros_interface.setCurrentIndex(max(0, self.ros_interface.findData(ros.interface_mode)))
        self.ros_namespace = QLineEdit(ros.namespace)
        self.ros_cmd_vel = QLineEdit(ros.cmd_vel_topic)
        self.ros_drive_action = QLineEdit(ros.drive_action_topic)
        self.ros_rotate_action = QLineEdit(ros.rotate_action_topic)
        self.ros_odom = QLineEdit(ros.odom_topic)
        self.ros_bumper = QLineEdit(ros.bumper_topic)
        self.ros_prefix = QLineEdit(ros.node_name_prefix)
        self.ros_linear = self._double_spin(0.01, 3.0, ros.linear_speed, 3)
        self.ros_angular = self._double_spin(0.05, 6.0, ros.angular_speed, 3)
        self.ros_cell = self._double_spin(0.01, 10.0, ros.cell_size_m, 3)
        self.ros_timeout = self._double_spin(0.1, 120.0, ros.command_timeout_s, 1)
        ros_form.addRow("Control interface", self.ros_interface)
        ros_form.addRow("Namespace template", self.ros_namespace)
        ros_form.addRow("Velocity topic", self.ros_cmd_vel)
        ros_form.addRow("Drive action", self.ros_drive_action)
        ros_form.addRow("Rotate action", self.ros_rotate_action)
        ros_form.addRow("Odometry topic", self.ros_odom)
        ros_form.addRow("Bumper topic", self.ros_bumper)
        ros_form.addRow("Node prefix", self.ros_prefix)
        ros_form.addRow("Linear speed", self.ros_linear)
        ros_form.addRow("Angular speed", self.ros_angular)
        ros_form.addRow("Grid cell size", self.ros_cell)
        ros_form.addRow("Command timeout", self.ros_timeout)
        ...
        self.ros_interface.currentIndexChanged.connect(self._update_environment_controls)
    ...
    def _update_environment_controls(self) -> None:
        ...
        action_mode = self.ros_interface.currentData() == "irobot_actions"
        self.ros_cmd_vel.setEnabled(not action_mode)
        self.ros_drive_action.setEnabled(action_mode)
        self.ros_rotate_action.setEnabled(action_mode)
    ...
    def configuration(self) -> SimulationConfig:
        config = SimulationConfig(
            ...
            ros=RosConfig(
                interface_mode=str(self.ros_interface.currentData()),
                namespace=self.ros_namespace.text().strip() or "/tb{index}",
                cmd_vel_topic=self.ros_cmd_vel.text().strip() or "cmd_vel",
                drive_action_topic=self.ros_drive_action.text().strip() or "drive_distance",
                rotate_action_topic=self.ros_rotate_action.text().strip() or "rotate_angle",
                odom_topic=self.ros_odom.text().strip() or "odom",
                bumper_topic=self.ros_bumper.text().strip() or "hazard_detection",
                node_name_prefix=self.ros_prefix.text().strip() or "actr_turtle_bridge",
                linear_speed=self.ros_linear.value(),
                angular_speed=self.ros_angular.value(),
                cell_size_m=self.ros_cell.value(),
                command_timeout_s=self.ros_timeout.value(),
            ),
...
class MultiSimulationRunView(QFrame):
    ...
    def _environment_changed(self, row: _ScenarioWidgets) -> None:
        ...
        if mode == "ros":
            row.config.human_agent_enabled = False
            row.scheduling.setCurrentIndex(max(0, row.scheduling.findData("sequential")))
            row.scheduling.setEnabled(False)
        else:
