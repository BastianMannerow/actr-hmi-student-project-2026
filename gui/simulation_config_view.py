"""GUI editor for persistent simulation, TurtleBot environment and plug-in settings."""
...
class SimulationConfigView(QFrame):
    ...
    def __init__(self, initial_config: SimulationConfig, parent=None) -> None:
        ...
        self.ros_group = self._build_ros_group()
        content_layout.addWidget(self.ros_group)
    ...
    def _build_world_group(self) -> QGroupBox:
        ...
        form.addRow("Level / ROS shadow map", self.virtual_level_combo)
    ...
    def _build_ros_group(self) -> QGroupBox:
        group = QGroupBox("ROS 2 TurtleBot Connection")
        form = QFormLayout(group)
        ros = self._current_config.ros
        self.ros_interface_combo = QComboBox(group)
        self.ros_interface_combo.addItem("iRobot Create actions (repository)", "irobot_actions")
        self.ros_interface_combo.addItem("Generic cmd_vel", "cmd_vel")
        self.ros_interface_combo.setCurrentIndex(
            max(0, self.ros_interface_combo.findData(ros.interface_mode))
        )
        self.ros_namespace_edit = QLineEdit(ros.namespace, group)
        self.ros_cmd_vel_edit = QLineEdit(ros.cmd_vel_topic, group)
        self.ros_drive_action_edit = QLineEdit(ros.drive_action_topic, group)
        self.ros_rotate_action_edit = QLineEdit(ros.rotate_action_topic, group)
        self.ros_odom_edit = QLineEdit(ros.odom_topic, group)
        self.ros_bumper_edit = QLineEdit(ros.bumper_topic, group)
        self.ros_node_prefix_edit = QLineEdit(ros.node_name_prefix, group)
        self.ros_linear_spin = self._double_spin(0.01, 3.0, ros.linear_speed, 3)
        self.ros_angular_spin = self._double_spin(0.05, 6.0, ros.angular_speed, 3)
        self.ros_cell_size_spin = self._double_spin(0.01, 10.0, ros.cell_size_m, 3)
        self.ros_timeout_spin = self._double_spin(0.1, 120.0, ros.command_timeout_s, 1)
        form.addRow("Control interface", self.ros_interface_combo)
        form.addRow("Namespace template", self.ros_namespace_edit)
        form.addRow("Velocity topic", self.ros_cmd_vel_edit)
        form.addRow("Drive action", self.ros_drive_action_edit)
        form.addRow("Rotate action", self.ros_rotate_action_edit)
        form.addRow("Odometry topic", self.ros_odom_edit)
        form.addRow("Bumper topic", self.ros_bumper_edit)
        form.addRow("Node prefix", self.ros_node_prefix_edit)
        form.addRow("Linear speed (m/s)", self.ros_linear_spin)
        form.addRow("Angular speed (rad/s)", self.ros_angular_spin)
        form.addRow("Grid cell size (m)", self.ros_cell_size_spin)
        form.addRow("Command timeout (s)", self.ros_timeout_spin)
        note = QLabel(
            "ROS mode uses the repository-compatible iRobot Create actions by default, with generic cmd_vel as an alternative. The selected level remains active as the GUI/perception shadow map."
        )
        note.setWordWrap(True)
        note.setObjectName("muted")
        form.addRow(note)
        self.ros_interface_combo.currentIndexChanged.connect(self._ros_interface_changed)
        return group

    def _ros_interface_changed(self) -> None:
        action_mode = self.ros_interface_combo.currentData() == "irobot_actions"
        self.ros_cmd_vel_edit.setEnabled(not action_mode)
        self.ros_drive_action_edit.setEnabled(action_mode)
        self.ros_rotate_action_edit.setEnabled(action_mode)
    ...
    def _environment_mode_changed(self) -> None:
        ros_mode = self.environment_mode_combo.currentData() == "ros"
        self.ros_group.setVisible(ros_mode)
        self.virtual_level_combo.setEnabled(True)
        self._virtual_level_changed()
        self.human_group.setEnabled(not ros_mode)
        if ros_mode:
            self.human_enabled_check.setChecked(False)
        self._ros_interface_changed()
    ...
    def collect_config(self, *, execution_mode: str, speed_factor: float) -> SimulationConfig:
        config = SimulationConfig(
            ...
            ros=RosConfig(
                interface_mode=str(self.ros_interface_combo.currentData()),
                namespace=self.ros_namespace_edit.text().strip() or "/tb{index}",
                cmd_vel_topic=self.ros_cmd_vel_edit.text().strip() or "cmd_vel",
                drive_action_topic=self.ros_drive_action_edit.text().strip() or "drive_distance",
                rotate_action_topic=self.ros_rotate_action_edit.text().strip() or "rotate_angle",
                odom_topic=self.ros_odom_edit.text().strip() or "odom",
                bumper_topic=self.ros_bumper_edit.text().strip() or "hazard_detection",
                node_name_prefix=self.ros_node_prefix_edit.text().strip() or "actr_turtle_bridge",
                linear_speed=self.ros_linear_spin.value(),
                angular_speed=self.ros_angular_spin.value(),
                cell_size_m=self.ros_cell_size_spin.value(),
                command_timeout_s=self.ros_timeout_spin.value(),
            ),
    ...
    def apply_config(self, config: SimulationConfig) -> None:
        ...
        self.ros_interface_combo.setCurrentIndex(max(0, self.ros_interface_combo.findData(config.ros.interface_mode)))
        self.ros_namespace_edit.setText(config.ros.namespace)
        self.ros_cmd_vel_edit.setText(config.ros.cmd_vel_topic)
        self.ros_drive_action_edit.setText(config.ros.drive_action_topic)
        self.ros_rotate_action_edit.setText(config.ros.rotate_action_topic)
        self.ros_odom_edit.setText(config.ros.odom_topic)
        self.ros_bumper_edit.setText(config.ros.bumper_topic)
        self.ros_node_prefix_edit.setText(config.ros.node_name_prefix)
        self.ros_linear_spin.setValue(config.ros.linear_speed)
        self.ros_angular_spin.setValue(config.ros.angular_speed)
        self.ros_cell_size_spin.setValue(config.ros.cell_size_m)
        self.ros_timeout_spin.setValue(config.ros.command_timeout_s)
