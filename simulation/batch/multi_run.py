...
class MultiRunScenario:
    ...
    def validate(self) -> None:
        ...
        if self.config.environment_mode == "ros" and self.scheduling != "sequential":
            raise ValueError(
                f"ROS scenario '{self.name}' must run sequentially because it controls physical hardware."
            )
    ...
    try:
        ...
        for event_index in range(max_events):
            ...
            if config.environment_mode == "ros":
                wait_deadline = time.monotonic() + config.ros.command_timeout_s * 2.5
                while simulation.run_state == "external_motion" and time.monotonic() < wait_deadline:
                    time.sleep(0.05)
                if simulation.run_state == "external_motion":
                    raise TimeoutError("ROS movement did not finish before the batch timeout.")
